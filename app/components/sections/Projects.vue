...
onMounted(() => {
    // Header animation
    if (headerRef.value) {
        fadeIn(headerRef.value, { y: 30, duration: 0.8 })
    }

    // Projects grid stagger
    if (projectsGridRef.value) {
        const cards = projectsGridRef.value.querySelectorAll('.project-card')
        staggerIn(cards as any, { stagger: 0.2, y: 40, duration: 0.8 })
    }

    // CTA footer slide in
    if (ctaRef.value) {
        slideIn(ctaRef.value, 'bottom', { y: 40, duration: 0.8 })
    }
})
</script>
        ...
        <div class="container mx-auto max-w-350">
            ...
            <div ref="ctaRef"
                ...
                <a href="https://github.com/judempoyo" target="_blank" rel="noopener noreferrer"
                    class="inline-flex items-center gap-2 px-6 py-3 bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 text-sm font-black uppercase rounded-md hover:bg-green-600 dark:hover:bg-green-500 hover:text-white dark:hover:text-white transition-all duration-200 shadow-sm hover:shadow-md">
