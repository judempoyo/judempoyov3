...
onMounted(() => {
    if (headerRef.value) {
        fadeIn(headerRef.value, { y: 30 })
    }

    if (projectsGridRef.value) {
        const cards = projectsGridRef.value.querySelectorAll('.project-card')
        staggerIn(cards as any, { stagger: 0.2, y: 40 })
    }

    if (ctaRef.value) {
        slideIn(ctaRef.value, 'bottom')
    }
})
</script>
...
<template>
    <section id="projects" aria-label="Featured Projects by Jude Mpoyo" class="py-24 px-6 bg-background dark:bg-zinc-900 text-foreground">
        <div class="container mx-auto max-w-350">
            ...
            <div ref="headerRef" class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        <span class="text-xs font-black text-green-600 dark:text-green-500 uppercase tracking-widest-2xl">
                            {{ t('projects.section_id') }}
                        </span>
                        <div class="h-px w-12 bg-green-600 dark:bg-green-500"></div>
                    </div>
            ...
            <div ref="projectsGridRef" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <ProjectSheet v-for="project in projects" :key="project.id" :project="project"
                    class="project-card bg-muted/10 dark:bg-zinc-800 border border-border dark:border-zinc-700 rounded-md p-6 transition-colors" />
            </div>
            ...
            <div ref="ctaRef"
                ...
                <a href="https://github.com/judempoyo" target="_blank" rel="noopener noreferrer"
                    class="inline-flex items-center gap-2 px-6 py-3 bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 text-sm font-black uppercase rounded-md hover:bg-green-600 dark:hover:bg-green-500 hover:text-white dark:hover:text-white transition-[transform,box-shadow,background-color,color] duration-200 shadow-sm hover:shadow-md hover:scale-[1.02] will-change-transform">
