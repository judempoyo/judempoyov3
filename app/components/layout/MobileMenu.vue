...
<template>
  ...
  >
    <div
      ...
      v-show="isOpen"
      class="md:hidden absolute top-14 left-0 w-full min-h-[50vh] bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl border-b border-zinc-200 dark:border-zinc-700 p-4 sm:p-6 flex flex-col gap-5 sm:gap-6 overflow-hidden rounded-b-2xl text-zinc-800 dark:text-zinc-200"
    >
      <div class="absolute top-0 right-0 p-4 opacity-20 pointer-events-none">
        <pre
          class="text-[8px] leading-tight font-mono uppercase text-zinc-500 dark:text-zinc-600"
          >{{ $t("mobile_menu.layout_init") }}
...
{{ $t("mobile_menu.state") }}</pre
      ...
      <div class="mt-auto space-y-5 pt-6 border-t border-zinc-200 dark:border-zinc-700">
        <div class="flex flex-col gap-4">
          ...
          <div
            class="flex items-center justify-between bg-zinc-100 dark:bg-zinc-800 p-3 rounded-lg"
          >
